...
// ใช้ค่าคงที่ให้ตรงกับแบ็กเอนด์ปัจจุบันของคุณ
...
export default function Profile() {
  ...
  const loadBookingHistory = async (uid) => {
    ...
    // 1) ลองเรียก endpoint ที่คาดว่าน่าจะมีอยู่
    const candidates = [
    ...
    // 2) แผนสำรอง: ไล่ย้อนหลัง 30 วัน โดยใช้ /api/bookings/my/:userId/:date
  ...
  // โหลดประวัติหลังรู้ userId
  useEffect(() => {
    if (userId) loadBookingHistory(userId);
  }, [userId]);

  // ฟัง BroadcastChannel / localStorage เพื่อรีโหลดประวัติเมื่อจองสำเร็จจากหน้า Details
  useEffect(() => {
    let bc;
    const onStorage = (e) => {
